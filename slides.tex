...
\item Turn off slow start \only<3->{\hspace{1.75 cm} \raisebox{-0.6ex}{\includegraphics[height=14 pt]{Google_Logo_Old.PNG}} \raisebox{-.125 ex}{\includegraphics[height=10 pt]{mslogo.png}}}
...
\item \textbf{\textcolor{DarkBlue}{Approximates}} solution by paring down search

\item Don't store full history
...
\begin{frame}
\frametitle{A RemyCC stores three state variables}

\Large

\noindent \hspace{-0.75 cm}\begin{tabular}{ll}
\textbf{\textcolor{DarkBlue}{r\_ewma}}: & moving average of interval between acks \\

\textbf{\textcolor{DarkBlue}{s\_ewma}}: & \ldots between sender timestamps echoed in acks \\

\textbf{\textcolor{DarkBlue}{rtt\_ratio}}: & ratio of last RTT to smallest RTT so far \\
...
\begin{frame}
\frametitle{Each state mapped to an {\bf action}}
...
\textcolor{Red}{$r$} & \textcolor{DarkBlue}{Minimum interval between two outgoing packets} \\
...
\begin{itemize}
\item Update $\{\textcolor{Red}{m}, \textcolor{Red}{b}, \textcolor{Red}{r}\} \leftarrow \textbf{Rule}[ \textcolor{DarkBlue}{r\_ewma}, \textcolor{DarkBlue}{s\_ewma}, \textcolor{DarkBlue}{rtt\_ratio} ]$.
